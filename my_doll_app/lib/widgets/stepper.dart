...
const EdgeInsets _stepsMargin = EdgeInsets.symmetric(vertical: 16, horizontal: 8);
...
class EJStep {
  ...
  final Function(int)? onStepOpen;
  final Function(int)? onUnfocused;
...
class EJStepper extends StatefulWidget {
  ...
  /// Color of leading when step's state is [EJStepState.checking].
  final Color? leadingCheckingColor;
...
class _EJStepperState extends State<EJStepper> {
  ...
  Color leadingColor(int index) {
    switch (widget.steps[index].state) {
      ...
      case EJStepState.checking:
        return widget.leadingCheckingColor ?? Colors.orange;
  ...
  void _changeStep(int index) {
    widget.steps[_currentStep].onUnfocused?.call(_currentStep);
    widget.steps[index].onStepOpen?.call(index);
  ...
  Widget _buildStep(int index) => InkWell(
    ...
    onTap: () {
      if (index != _currentStep) {
        ...
        widget.steps[index].onStepTap?.call(index);
      }
  ...
  Widget _buildTitle(int index) => Column(
    ...
    children: [
      ...
      if (widget.steps[index].error != null)
        Text(widget.steps[index].error!, style: TextStyle(color: Colors.redAccent, fontSize: 12),),
    ],
  );
  ...
  Widget _buildNextButton(int index) {
    ...
    return widget.nextButtonBuilder != null
        ...
        : GestureDetector(onTap: index == widget.steps.length - 1 && !isAllCompleted()?setError:() => _onNext(index), child: child);
  }

  void setError() {
    widget.steps.where((element) => element.state != EJStepState.complete).forEach((element) {
      setState(() {
        element.state = EJStepState.error;
      });
    });
  }
  bool isAllCompleted() => widget.steps.where((element) => element.state != EJStepState.complete).isEmpty;
